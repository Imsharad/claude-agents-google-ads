...
class PersonaOptimizer:
    ...
    def _get_campaign_details(self, customer_id: str, campaign_id: str) -> Dict[str, Any]:
        ...
        """
        response = self.google_ads_service.search_stream(customer_id=customer_id, query=query)
    ...
    def _get_ad_group_performance(self, customer_id: str, campaign_id: str) -> List[Dict[str, Any]]:
    ...
    def identify_losing_personas(self, customer_id: str, campaign_id: str) -> List[str]:
        ...
        for ad_group in ad_groups:
            ...
            if ad_group["conversions"] == 0 and ad_group["cost_micros"] > 2000 * 1_000_000:
                is_losing = True

            if is_losing:
                losing_ad_groups.append(ad_group["ad_group_id"])
                logging.info(f"Identified losing ad group {ad_group['ad_group_id']} (Cost: {ad_group['cost_micros']}, Conversions: {ad_group['conversions']}, CPA: {ad_group['cost_per_conversion']})")

        return losing_ad_groups

    def identify_winning_personas(self, customer_id: str, campaign_id: str) -> List[str]:
        """
        ...
        if not target_cpa or target_cpa == 0:
            logging.warning(f"Cannot identify winning personas for campaign {campaign_id} without a target CPA.")
            return []

        for ad_group in ad_groups:
            # Criteria: cost_per_conversion < target_cpa AND conversions >= 5
            if ad_group["cost_per_conversion"] < target_cpa and ad_group["conversions"] >= 5:
                winning_ad_groups.append(ad_group["ad_group_id"])
                logging.info(f"Identified winning ad group {ad_group['ad_group_id']} (Cost: {ad_group['cost_micros']}, Conversions: {ad_group['conversions']}, CPA: {ad_group['cost_per_conversion']})")
    ...
    def _get_ad_group_details(self, customer_id: str, ad_group_id: str) -> Dict[str, Any]:
    ...
    def _update_keyword_bids(self, customer_id: str, ad_group_id: str, percentage: float):
        ...
        if not operations:
            logging.warning(f"No keywords with existing bids found to update for ad group {ad_group_id}.")
            return
        ...
        try:
            ...
            logging.info(f"Successfully increased bids by {percentage:.2%} for {len(operations)} keywords in ad group {ad_group_id}.")
        except Exception as e:
            logging.error(f"Failed to update keyword bids for ad group {ad_group_id}: {e}")
            raise


    def increase_bids(self, customer_id: str, ad_group_id: str, percentage: float = 0.20):
        """
        ...
        bidding_strategy_enum = self.client.get_type("BiddingStrategyTypeEnum").BiddingStrategyType

        if bidding_strategy == bidding_strategy_enum.MANUAL_CPC:
            logging.info(f"Bidding strategy is Manual CPC. Increasing keyword bids for ad group {ad_group_id}.")
            ...
            if not current_limit:
                logging.warning(f"Campaign {campaign_id} has no CPC bid limit. Cannot increase.")
                return
            ...
            self.client.copy_from(
                ...
                self.client.get_type("FieldMask", paths=["maximize_clicks.cpc_bid_limit_micros"]),
            )

            try:
                campaign_service.mutate_campaigns(customer_id=customer_id, operations=[campaign_operation])
                logging.info(f"Successfully increased Maximize Clicks bid cap to {new_limit} for campaign {campaign_id}.")
            except Exception as e:
                logging.error(f"Failed to increase bid cap for campaign {campaign_id}: {e}")
                raise
        elif bidding_strategy in [bidding_strategy_enum.TARGET_CPA, bidding_strategy_enum.TARGET_ROAS]:
            logging.info(f"Bidding strategy is {bidding_strategy.name}. No action taken as it is an automated strategy.")
        else:
