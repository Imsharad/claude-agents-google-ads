...
class TestPersonaOptimizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_get_client = patch(
            "src.optimization.persona_optimizer.get_google_ads_client",
    ...
    def test_identify_losing_personas_by_cpa(self):
        ...
        self.optimizer._get_campaign_details = MagicMock(
            return_value={"target_cpa_micros": 10000000}
        )  # 10 currency units
        # Mock performance data with one ad group having a high CPA
        mock_performance_data = [
            {
                "ad_group_id": "1",
                "cost_per_conversion": 15000000,
                "conversions": 2,
                "cost_micros": 30000000,
            },
            {
                "ad_group_id": "2",
                "cost_per_conversion": 8000000,
                "conversions": 5,
                "cost_micros": 40000000,
            },
        ]
        self.optimizer._get_ad_group_performance = MagicMock(
            return_value=mock_performance_data
        )

        losing_personas = self.optimizer.identify_losing_personas(
    ...
    def test_identify_winning_personas(self):
        ...
        winning_personas = self.optimizer.identify_winning_personas(
            self.customer_id, self.campaign_id
        )
    ...
    def test_increase_bids_manual_cpc(self):
        ...
        ad_group_id = "1"
        bidding_strategy_enum = (
            self.mock_client.get_type.return_value.BiddingStrategyType
        )
        self.optimizer._get_ad_group_details = MagicMock(
            return_value={"bidding_strategy_type": bidding_strategy_enum.MANUAL_CPC}
        )
        ...
        mock_google_ads_service.search_stream.return_value = [
            MagicMock(
                results=[
                    MagicMock(
                        ad_group_criterion=MagicMock(
                            resource_name="keyword1", cpc_bid_micros=100000
                        )
                    )
                ]
            )
